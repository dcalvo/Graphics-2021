...
template <>
void GLSLProgram::glUniformiv<1>(GLint location, GLsizei count, const GLint* value) {
	glUniform1iv(location, count, value);
}

template <>
void GLSLProgram::glUniformiv<2>(GLint location, GLsizei count, const GLint* value) {
	glUniform2iv(location, count, value);
}

template <>
void GLSLProgram::glUniformiv<3>(GLint location, GLsizei count, const GLint* value) {
	glUniform3iv(location, count, value);
}

template <>
void GLSLProgram::glUniformiv<4>(GLint location, GLsizei count, const GLint* value) {
	glUniform4iv(location, count, value);
}

template <>
void GLSLProgram::glUniformfv<1>(GLint location, GLsizei count, const GLfloat* value) {
	glUniform1fv(location, count, value);
}

template <>
void GLSLProgram::glUniformfv<2>(GLint location, GLsizei count, const GLfloat* value) {
	glUniform2fv(location, count, value);
}

template <>
void GLSLProgram::glUniformfv<3>(GLint location, GLsizei count, const GLfloat* value) {
	glUniform3fv(location, count, value);
}

template <>
void GLSLProgram::glUniformfv<4>(GLint location, GLsizei count, const GLfloat* value) {
	glUniform4fv(location, count, value);
}

template <>
void GLSLProgram::glUniformMatrixfv<2>(GLint location, GLsizei count, GLboolean transpose, const GLfloat* value) {
	GLint id;
	glGetIntegerv(GL_CURRENT_PROGRAM, &id);
	if (!id)
		WARN("No current program");
	else
		glUniformMatrix2fv(location, count, transpose, value);
}

template <>
void GLSLProgram::glUniformMatrixfv<3>(GLint location, GLsizei count, GLboolean transpose, const GLfloat* value) {
	GLint id;
	glGetIntegerv(GL_CURRENT_PROGRAM, &id);
	if (!id)
		WARN("No current program");
...
template <>
void GLSLProgram::glUniformMatrixfv<4>(GLint location, GLsizei count, GLboolean transpose, const GLfloat* value) {
	GLint id;
	glGetIntegerv(GL_CURRENT_PROGRAM, &id);
	if (!id)
		WARN("No current program");
...
GLSLProgram::GLSLProgram(const std::string& vs_filename, const std::string& fs_filename) : _handle(0), _linked(false) {
	...
	if (GetShaderType(vs_filename) != GL_VERTEX_SHADER)
		THROW("Expected vertex shader: %s", vs_filename.c_str());
	if (!FileExists(vs_filename))
		THROW("Vertex shader not found: %s", vs_filename.c_str());
	{
		std::ifstream inFile(vs_filename, std::ios::in);
		if (!inFile)
			THROW("Unable to open vertex shader: %s", vs_filename.c_str());
	...
	if (GetShaderType(fs_filename) != GL_FRAGMENT_SHADER)
		THROW("Expected fragment shader: %s", fs_filename.c_str());
	if (!FileExists(fs_filename))
		THROW("Fragment shader not found: %s", fs_filename.c_str());
	{
		std::ifstream inFile(fs_filename, std::ios::in);
		if (!inFile)
			THROW("Unable to open fragment shader: %s", fs_filename.c_str());
		std::stringstream code;
		code << inFile.rdbuf();
		inFile.close();
...
GLenum GLSLProgram::GetShaderType(const std::string& fileName) {
	...
	for (int i = 0; i < numExts; i++) if (ext == GLSLShaderInfo::extensions[i].ext) return GLSLShaderInfo::extensions[i]
		.type;
...
void GLSLProgram::link(void) {
	if (_linked) return;
	if (_handle <= 0)
		THROW("Program has not been compiled.");
...
void GLSLProgram::setUniform(const std::string& name, int val, bool showWarning) {
	...
	if (loc >= 0)
		...
		WARN("Non-existant uniform: %s", name.c_str());
}
