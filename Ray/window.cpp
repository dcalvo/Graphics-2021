...
Timer Window::frameCountTimer;

Timer Window::frameTimer;
...
int Window::PrintError(int showNoError) {
	...
	int e = 1;
	switch (glGetError()) {
	case GL_NO_ERROR: e = 0;
		break;
	case GL_INVALID_ENUM: std::cout << "Invalid Enum" << std::endl;
		break;
	case GL_INVALID_VALUE: std::cout << "Invalid Value" << std::endl;
		break;
	case GL_INVALID_OPERATION: std::cout << "Invalid Operation" << std::endl;
		break;
	case GL_STACK_OVERFLOW: std::cout << "Stack Overflow" << std::endl;
		break;
	case GL_STACK_UNDERFLOW: std::cout << "Stack Underflow" << std::endl;
		break;
	case GL_OUT_OF_MEMORY: std::cout << "Out of memory" << std::endl;
		break;
	}
...
Image32 Window::TakeSnapshot(void) {
	...
	auto pixels = new GLfloat[vp[2] * vp[3] * 3];
	if (!pixels) return img;
	img.setSize(vp[2], vp[3]);
	glReadBuffer(GL_FRONT);
	glReadPixels(vp[0], vp[1], vp[2], vp[3], GL_RGB, GL_FLOAT, pixels);

	for (int i = 0; i < vp[3]; i++)
		for (int j = 0; j < vp[2]; j++) {
			int idx;
			Pixel32 p;
			idx = 0 + j * 3 + (vp[3] - i - 1) * (vp[2]) * 3;
			p.r = 255 * pixels[idx];
			idx = 1 + j * 3 + (vp[3] - i - 1) * (vp[2]) * 3;
			p.g = 255 * pixels[idx];
			idx = 2 + j * 3 + (vp[3] - i - 1) * (vp[2]) * 3;
			p.b = 255 * pixels[idx];
			img(j, i) = p;
		}
	delete[] pixels;
...
void Window::MotionFunction(int x, int y) {
	...
	if (mouse.middleDown || (mouse.leftDown && mouse.ctrlDown))
		scene->_globalData.camera.rotateUp(center, 0.01 * d[0]),
			scene->_globalData.camera.rotateRight(center, 0.01 * d[1]);
	else if (mouse.scrollDown || (mouse.leftDown && mouse.shiftDown))
		scene->_globalData.camera.moveForward(
			radius / 15 * d[1]);
	else if (mouse.leftDown)
		scene->_globalData.camera.moveRight(-radius / 250 * d[0]), scene->_globalData.camera.
		                                                                  moveUp(radius / 250 * d[1]);
	...
	case WRITE_SCENE:
		{
			...
			if (!ostream)
				THROW("Failed to open file for writing: %s\n", fileName.c_str());
			ostream << *scene;
			break;
		}
	default:
...
void Window::DisplayFunction(void) {
	...
	gluPerspective(scene->_globalData.camera.heightAngle * 180. / Pi, static_cast<double>(_width) / _height, .1 * d,
	               2 * d);
	...
	if (frameRateLimit > 0) {
		double frameRateMargin = 1000. / frameRateLimit - frameTimer.elapsed();
		if (frameRateMargin > 0)
			std::this_thread::sleep_for(std::chrono::duration<double, milli>(frameRateMargin));
	}
	...
	if (scene->_globalData.shader && scene->_globalData.shader->glslProgram)
		scene->_globalData.shader->glslProgram->
		       use();

	// Swap the back and front buffers
	glutSwapBuffers();

	// Write out a screenshot
	if (frameRateLimit > 0 && gifLength > 0) {
		Image32 img;
		std::string name = std::to_string(frameCount) + ".jpg";

		// take screenshot without setting the read buffer
		GLint vp[4];
		glGetIntegerv(GL_VIEWPORT, vp);

		auto pixels = new GLfloat[vp[2] * vp[3] * 3];
		if (!pixels) {
			cout << "error taking screenshot during gif generation" << '\n';
			return;
		}
		img.setSize(vp[2], vp[3]);
		//glReadBuffer(GL_FRONT);
		...
		img.write(name);
		if (frameCount % frameRateLimit == 0) gifLength--;
		if (gifLength == 0) cout << "gif images generated" << '\n';
	}
}
...
void Window::View(Scene& s, int width, int height) {
	...
	int argc = 1;
	auto argv = new char[strlen("hello") + 1];
	strcpy(argv, "hello");
	glutInit(&argc, &argv);
	delete[] argv;
