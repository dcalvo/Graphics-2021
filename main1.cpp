...
void ShowUsage(const string& ex) {
	...
	cout << "\t[--" << RandomDither.name << " <bits per channel with random dithering>=" << RandomDither.value << "]" <<
		endl;
	cout << "\t[--" << OrderedDither2X2.name << " <bits per channel with ordered dithering>=" << OrderedDither2X2.value
		<< "]" << endl;
	cout << "\t[--" << FloydSteinbergDither.name << " <bits per channel with Floyd-Steinberg dithering>=" <<
		FloydSteinbergDither.value << "]" << endl;
	...
	try {
		...
		if (BeierNeelyMorph.set) {
			...
			try { istream >> olsp; }
				...
				THROW("failed to read OrientedLineSegmentPairs: %s\n%s", BeierNeelyMorph.values[1].c_str(), e.what());
			}
